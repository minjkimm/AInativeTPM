...
async function sample(request: Request, file: string) {
  const response = await fetch(new URL(`/mock/${file}`, request.url));
  if (!response.ok) throw new Error(`Sample source ${file} returned ${response.status}`);
...
async function withFallback(
  ...
  if (!isConfigured) {
    const data = await sample(request, file);
...
async function jiraConnector(request: Request): Promise<ConnectorResult> {
  const base = process.env.JIRA_BASE_URL?.replace(/\/$/, "");
  const email = process.env.JIRA_EMAIL;
  const token = process.env.JIRA_API_TOKEN;
  return withFallback(request, "Jira", "jira-issues.json", Boolean(base && email && token), async () => {
    const fields = [
...
async function smartsheetConnector(request: Request): Promise<ConnectorResult> {
  const token = process.env.SMARTSHEET_TOKEN;
  const sheetId = process.env.SMARTSHEET_SHEET_ID;
  return withFallback(request, "Smartsheet", "smartsheet-activations.json", Boolean(token && sheetId), async () => {
...
async function googleSheetsConnector(request: Request): Promise<ConnectorResult> {
  const sheetId = process.env.GOOGLE_SHEET_ID;
  const apiKey = process.env.GOOGLE_SHEETS_API_KEY;
  const range = process.env.GOOGLE_BUDGET_RANGE || "FY27 Q3 Budget!A1:H";
  return withFallback(request, "Google Sheets", "google-sheet-budget.json", Boolean(sheetId && apiKey), async () => {
...
async function documentsConnector(request: Request): Promise<ConnectorResult> {
  const folderId = process.env.GOOGLE_DRIVE_FOLDER_ID;
  const token = process.env.GOOGLE_DRIVE_ACCESS_TOKEN;
  return withFallback(request, "Documents", "playbook-documents.json", Boolean(folderId && token), async () => {
...
export async function GET(request: Request) {
  try {
    const [jira, smartsheet, budget, documents] = await Promise.all([
      jiraConnector(request),
      smartsheetConnector(request),
      googleSheetsConnector(request),
      documentsConnector(request),
    ]);
